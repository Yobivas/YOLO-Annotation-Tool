(() => {
    ...
    const backgroundColor = "rgba(0, 116, 217, 0.05)" // Base bbox fill color
    const markedFontColor = "#FF4136" // Marked bbox font color
    const markedBorderColor = "#FF4136" // Marked bbox border color
    const markedBackgroundColor = "rgba(255, 133, 27, 0.05)" // Marked bbox fill color
    const minBBoxWidth = 2 // Minimal width of bbox
    const minBBoxHeight = 2 // Minimal height of bbox
    const scrollSpeed = 1.1 // Multiplying factor of wheel speed
    const minZoom = 0.1 // Smallest zoom allowed
    const maxZoom = 10 // Largest zoom allowed
    ...
    const setCurrentClass = () => {
        ...
        if (currentBbox !== null) {
            // Update the class of the highlighted bounding box in place
            const oldClass = currentBbox.bbox.class;
            currentBbox.bbox.class = currentClass;
            
            // Update the bboxes dictionary to reflect the class change
            bboxes[currentImage.name][oldClass].splice(currentBbox.index, 1);
